...
def build_human_detection_operator(i: int) -> KubernetesPodOperator:
    return KubernetesPodOperator(
        ...
        cmds=["bash", "-c"],
        arguments=[f"pip install opencv-python-headless && python3 /opt/airflow/dags/repo/example/mapreduce/mapreduce_video_utils.py detect {INPUT_FILE_PATTERN.format(i=i)} {OUTPUT_FILE_PATTERN.format(i=i)}"],
...
with DAG(
    ...
    generate_videos = KubernetesPodOperator(
        ...
        image=PYTORCH_GPU_IMAGE,
        cmds=["bash", "-c"],
        arguments=[f"pip install opencv-python-headless && python3 /opt/airflow/dags/repo/example/mapreduce/mapreduce_video_utils.py generate {NUM_FILES} {INPUT_FILE_PATTERN}"],
        volumes=[volume, dags_volume],
        volume_mounts=[volume_mount, dags_volume_mount],
        container_resources=K8S_RESOURCES,
        get_logs=True,
